...
contract CCNCarnival2025 {
    ...
    struct Stall {
        ...
        // Ratings
        mapping(address => uint8) ratings; // user => rating (1-5)
        uint256 ratingsSum;
    ...
    modifier stallOpen(uint256 stallId) {
    ...
    function payStall(uint256 stallId) external payable stallOpen(stallId) {
        require(stallExists[stallId], "Stall does not exist");
    ...
    function refund(uint256 stallId, address to, uint256 amount) external onlyStallOwner(stallId) stallOpen(stallId) {
        Stall storage s = stalls[stallId];
    ...
    function getStall(uint256 stallId) external view returns (
        address owner, string memory name, Duration duration, uint256 totalFunds, uint256 withdrawTime, Status status, bool withdrawn
    ) {
        Stall storage s = stalls[stallId];
        return (s.owner, s.name, s.duration, s.totalFunds, s.withdrawTime, s.status, s.withdrawn);
    }
